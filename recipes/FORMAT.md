...
The game must match the recipe, each key must be declared by that recipe, and
levels must be integers from 1 through 100. Configuration changes only declared
single-byte cap sites after all fixed recipe writes have been validated. Unknown
keys, duplicate recipe sites, inconsistent generated defaults, booleans, and
out-of-range levels fail before an output is published. Canonical output hashes
remain mandatory for default configuration; a genuine override necessarily
produces a different reported output hash.
