...
import { invoke, browser, chooseFile, saveResult, clearDownloads } from "./platform";
import "./styles.css";
...
let busy = false;
let importedRandomized: Uint8Array | null = null;
...
function render(): void {
  const caps = config
    ? Object.entries(config.level_caps)
        ...
        .join("")
    : "";
  const randomizerReady = !randomizerEnabled || (browser
    ? Boolean(importedRandomized && importedManifest)
  ...
  app.innerHTML = `
    <section class="shell">
      <header>
        ...
        <span class="privacy">Your ROM stays on your device</span>
      </header>

      <section class="intro">
        <p>Choose your own backup of an exact supported English release. Choose your challenge settings, then download a patched copy. Your original backup stays unchanged. No ROM is uploaded.</p>
      </section>
        ...
        <section class="card options-card ${config ? "" : "disabled"}">
          ...
          <fieldset class="randomizer-fieldset" ${config ? "" : "disabled"}>
            ...
            <label class="preset-option">
              ...
              <span><strong>${browser ? "Use an FVX-randomized backup" : "Randomize before applying NuzLike"}</strong><small>${browser ? "Import a randomized ROM and its matching NuzLike FVX manifest." : "Uses the pinned GPL Universal Pokémon Randomizer FVX engine locally."}</small></span>
            </label>
            ${randomizerEnabled && browser ? `
              <p class="notice">The browser does not run Java FVX. Generate a randomized backup with the NuzLike FVX bridge, then select that backup and its matching manifest here. Keep your clean backup selected above.</p>
              <button id="choose-randomized" type="button">${importedRandomized ? "Randomized ROM selected — replace" : "Choose randomized ROM"}</button>
              <button id="choose-manifest" type="button">${importedManifest ? "FVX manifest selected — replace" : "Choose FVX manifest (.json)"}</button>
              <p class="preset-note">Both ROM fingerprints and compatibility are checked before patching. Fixed NuzLike caps do not change to match randomized trainers.</p>
            ` : randomizerEnabled ? `
              <label class="cap-row">
          ...
          <details id="boss-caps" ${config ? "" : "inert"}>
      ...
      <section class="action-bar">
        <div id="status" role="status" aria-live="polite" data-kind="idle">${selectedGame ? "Configuration ready" : "Choose a supported game backup to begin"}</div>
        <button id="patch-rom" class="primary" type="button" ${ready ? "" : "disabled"}>${browser ? "Build patched copy" : randomizerEnabled ? "Randomize, patch, and save" : "Patch and save copy"}</button>
      </section>

      <section id="downloads" aria-label="Built files"></section>
      <footer>
        <span>Supports Red, Blue, Yellow, Crystal, Emerald, FireRed, and LeafGreen.</span>
        <span>${escapeHtml(version)} · ${browser ? `<a href="./nuzlike-${escapeHtml(version)}-offline.html" download>Download offline patcher</a> · ` : ""}<a href="https://github.com/msmfai/nuzlike">Source and instructions</a> · GPL-3.0-or-later</span>
      </footer>
    </section>`;

  renderActionState();
  document.querySelectorAll<HTMLInputElement | HTMLTextAreaElement>(".options-card input, .options-card textarea").forEach(input => {
    input.addEventListener("input", () => {
      clearDownloads();
      setStatus("Options changed. Build a new patched copy to use them.");
    });
  });
  document.querySelector("#choose-randomized")?.addEventListener("click", () => importFvx(false));
  document.querySelector("#choose-manifest")?.addEventListener("click", () => importFvx(true));
  ...
  document.querySelectorAll<HTMLInputElement>(".cap-input").forEach((input) => {
    input.addEventListener("change", () => {
      ...
      if (Number.isInteger(value) && value >= 1 && value <= 100) {
        ...
        render();
        document.querySelector<HTMLDetailsElement>("#boss-caps")?.setAttribute("open", "");
      } else {
...
function renderActionState(): void {
  ...
  button.disabled = busy || !selectedBytes || !selectedGame?.recipeId || !config
    || (randomizerEnabled && (browser ? !importedRandomized || !importedManifest : !seedValid || !randomizerSettings));
}

function setBusy(value: boolean): void {
  busy = value;
  document.querySelectorAll<HTMLInputElement | HTMLButtonElement | HTMLTextAreaElement>("button, input, textarea").forEach(element => { element.disabled = value; });
  renderActionState();
}

async function importFvx(manifest: boolean): Promise<void> {
  if (busy) return;
  setBusy(true);
  try {
    const file = await chooseFile(manifest ? ".json" : ".gb,.gbc,.gba", manifest ? 1024 * 1024 : undefined);
    if (!file) return;
    if (manifest) {
      const text = new TextDecoder("utf-8", { fatal: true }).decode(file.bytes);
      JSON.parse(text);
      importedManifest = text;
    } else importedRandomized = file.bytes;
    render();
...
async function chooseRom(): Promise<void> {
  ...
  setBusy(true);
  try {
    const file = await chooseFile(".gb,.gbc,.gba");
    if (!file) return;
    setBusy(true);
...
async function patchAndSave(): Promise<void> {
  if (busy || !selectedGame?.recipeId || !selectedBytes || !config) return;
  setBusy(true);
  clearDownloads();
  try {
    ...
    if (randomizerEnabled) {
      setStatus(browser ? "Validating your FVX files…" : "Randomizing locally with FVX…", "working");
      const clean = selectedNormalization === "removed-512-byte-copier-header"
        ? selectedBytes.slice(512) : selectedBytes;
      let randomized: { metadata: FvxMetadata; bytes: Uint8Array };
      if (browser) {
        if (!importedRandomized || !importedManifest) throw new Error("Choose both the randomized ROM and its FVX manifest.");
        randomized = { bytes: importedRandomized, metadata: { manifestJson: importedManifest, log: "" } };
      } else {
        const request = encodeRawRequest({ settings: randomizerSettings, seed: randomizerSeed }, clean);
        randomized = decodeRawResponse<FvxMetadata>(await invoke<ArrayBuffer>("randomize_with_fvx", request));
    ...
    if (combinedManifestText) sidecarFiles[".nuzlike.json"] = combinedManifestText;
    if (randomizerLog !== null) sidecarFiles[".fvx.log"] = randomizerLog;
    if (!await saveResult(`nuzlike-${selectedGame.id}.${extension}`, bytes, sidecarFiles)) {
    ...
    const randomizedLabel = randomizerEnabled ? " · FVX randomized" : "";
    const sidecars = combinedManifestText ? " · combined manifest included" : "";
    setStatus(`${browser ? "Ready — download your copy below" : "Saved safely"} · ${outputHash.slice(0, 12)}… · hardcore${randomizedLabel}${normalized}${sidecars}${cheats ? ` · ${cheats} debug cheat${cheats === 1 ? "" : "s"}` : ""}`, "success");
  } catch (error) {
    setStatus(String(error), "error");
  } finally {
    setBusy(false);
